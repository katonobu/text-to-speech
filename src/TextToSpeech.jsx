...
const TextToSpeech = ({ tracks }) => {
  ...
  return (
    <div>
      <button onClick={prevTrack} disabled={0 == currentTrack}>{"<<"}</button>
      <button onClick={handlePlayPause}>{playPauseButtonStr()}</button>
      <button onClick={stop} disabled={playingStt === "IDLE"}>STOP</button>
      <button onClick={nextTrack} disabled={!(currentTrack < tracks.length - 1)}>{">>"}</button>
      <p>{currentTrack+1}/{tracks.length}</p>
    </div>
  );
};
